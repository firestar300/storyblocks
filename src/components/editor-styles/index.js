...
export function EditorStyles({ forceThemeName }) {
  ...
  const currentTheme =
    getRegisteredThemes().find((theme) => theme.name === (forceThemeName || currentThemeName)) ||
    getRegisteredThemes()[0]
  return <BlockEditorStyles styles={[{ css: currentTheme.editorStyles }]} />
}

EditorStyles.propTypes = {
  forceThemeName: PropTypes.string,
}
